...
afterwards: when a team plays next, how an event is ranking, who picked whom,
what a district season looks like — as a table when you are reading it and as
JSON when something else is. Responses are cached and revalidated, so repeated
questions are cheap and a flaky venue network degrades to the last answer
...
- [Caching and network](#caching-and-network)
- [Scripting](#scripting)
- [Shell completion and man pages](#shell-completion-and-man-pages)
...
Install it — a prebuilt binary from the
[Releases](https://github.com/the-blue-alliance/tba-cli/releases) page, or from
source:
...
Get a read API key from your
[TBA account page](https://www.thebluealliance.com/account) and hand it over:

```
tba auth login
```

Then the three commands you will use most:

```
tba team next 177                        # when does 177 play next?
tba event matches 2024cthar --team 177   # 177's whole schedule at that event
tba event rankings 2024cthar             # how the event is ranking
```
...
[Releases](https://github.com/the-blue-alliance/tba-cli/releases) page.
...
**macOS.** The binaries are not notarized, so Gatekeeper may refuse to run one
you downloaded ("cannot be opened because the developer cannot be verified").
Clearing the quarantine attribute is enough:
...
[TBA account page](https://www.thebluealliance.com/account): sign in, then
create a read API key. Then:
...
tba auth status                # is a key stored, and does it work?
tba auth logout                # remove the stored key
```
...
Keys are stored per base URL in `auth.yaml` beside the config file, so a
production key and a local development key do not conflict.

## Everyday commands

### At an event

`tba team next <team> [event]` finds the next match a team has not played. With
no event it uses the one the team is at today, or the next one it is going to:
...
A match whose time has already passed reads `Overdue by` instead, and `--all`
prints everything still to play as the match table below.
...
next one: `try --year 2025`.

`tba team standing <team> [event]` is how one team stands at one event. The
event can also be given as `--event KEY`; with neither, the team's event for
today is used, or the next one it is going to, the same way `team next` works
it out:
...
The rows between `Record` and `Alliance` are the season's own ranking
tiebreakers, named and rounded the way the event reports them. The API
sometimes carries more numbers than the season has names for; the unnamed ones
are dropped rather than printed as an invented "Sort Order 6", which is what
`event rankings` does too. Each part of the answer only exists once that part of
the event has happened, so before it starts you get `not ranked yet`,
`not selected` and `not started` rather than blanks. A team that is not
attending the event has no standing there, which is exit 5.

#### Match listings

`tba event matches` and `tba team matches` print the same table, so what you
learn on one reads the same on the other:
...
Matches always come back in the order the event plays them — qualification
matches by number, then the elimination rounds — never alphabetically by key,
which would put `qm10` before `qm2` and the finals before the quarterfinals.

**A whole season.** `tba team matches 177 --year 2024` spans several events, and
every one of them has a Qual 12, so a season listing is grouped by event — in
the order the team competed, which costs one extra request for the team's event
list — behind an extra `Event` column holding the event key. `--event KEY`
narrows the listing to one event and drops that column.

**Labels.** The `Match` column is what the match is announced by; `Key` keeps
the raw key for scripts.
...
Which form a playoff match gets depends on the event's bracket, so these
commands fetch the event as well. If that fetch fails the listing still prints,
guessing from the season.

**Scores and status.** The API scores an unplayed match `-1` to `-1`. That is a
placeholder, not a result, so the score cell is left blank and `Status` reads
`Scheduled`. A played match with no winning alliance is a `tie`.
...
When a mark appears in `table` output, the legend `* surrogate  ! disqualified`
is printed to stderr, so it explains the table on screen without landing in a
...
Filters apply to `--format json` too, so `--jq` and `--format csv` always see
the same rows.

**Nothing to show.** An event whose schedule has not been posted, or a filter
...
`tba match view <key>` is one match in full: what it is called, when it is
relative to now, both alliances by driver station, the game's own score
breakdown, and links to any video.
...
The score breakdown follows, as one table with a column per alliance, because
what a breakdown is for is comparing the two:
...
The fields change every season and are documented nowhere, so they are ordered
...
`tba event watch <key>` polls an event and prints what changed since the last
look. It talks to a shared API for as long as it is left running, so the
defaults are deliberately timid: **one poll a minute, for two hours, then it
stops**. To run it longer, say so:
...
Unchanged polls are cheap: each one is a conditional request the API answers
with a 304 and no body, so a watch left open all afternoon costs far less than
its poll count suggests.

Output is append-only. Nothing is cleared, nothing is redrawn, and the cursor
never moves, so the stream can be scrolled back through, `tee`d into a file or
diffed later. The first poll prints the whole match table — the same columns as
`tba event matches` — and every later poll prints only the rows that changed,
under a line naming the time and the poll number:
...
`--team 177` narrows the whole feed to one team's matches. `--rankings` adds the
standings, printed after the matches on the first poll and again whenever a rank
or a record moves. A match counts as changed when it is newly played, when a
score or a winner changes, when its predicted time moves by a minute or more, or
when it appears in the schedule for the first time; `actual_time` being
restamped after the fact is not news.

Piping a watch into something that stops reading —
`tba event watch 2024cthar | head -1` — stops it too: a quiet poll writes
nothing, so a broken pipe would never surface on its own, and the watch checks
stdout before each poll and each wait instead, exiting 141 without a word. (Unix
only; on Windows it keeps polling until `--for` elapses.)

Watching an event that is already over stops at the first poll: once every match
has been played and the event's last day is past, there is nothing left to see,
and `note: 2024cthar ended 2024-03-24; nothing left to watch` says so before the
command exits 0.
...
Both exit 0. Ctrl-C exits 130 without a word. A poll that fails is a note on
stderr and another try at the next interval — five failures in a row is exit 1.

**JSON Lines.** Piped, or with `--format json`, each change is one
self-contained JSON object on its own line, flushed as it happens. It is never a
growing array, so a reader can act on a change the moment it arrives. The first
line is a snapshot of everything as it stood at the first poll:
...
`change` is one of `added`, `played`, `score`, `winner` or `rescheduled`. `--jq`
is applied to each line on its own, so a filter written for a single change works
all day:
...
`csv`, `tsv` and `markdown` describe a finished table and have nothing to say
about a stream, so they are a usage error. `--columns` and `--sort` do not apply
...
Searching every season is not offered: it would repeat that walk once per year,
so `--year 0` and `--all-years` are usage errors.
...
With [completion installed](#shell-completion-and-man-pages), event keys, match
keys and `--district` values are completed **from the local response cache
only** — completion never makes a request. A shell asks on every Tab, so going
to the network would mean rate limits and a stalled prompt. Run a command once
and its keys complete afterwards:
...
numbers, and a list of file names would be worse than nothing. Flags with a
fixed set of values — `--format`, `--color`, `--type`, `--to`, `--only` —
complete those values without touching the cache.
...
`event oprs` shows what TBA calculates each team contributed: OPR (offensive
power rating), DPR (defensive power rating) and CCWM (calculated contribution to
winning margin), highest OPR first because that is the question the command is
asked. `--sort` reorders it by any column, and ties keep team-number order.

```
tba event oprs 2024cthar
...
The insights commands all answer with documents TBA computes rather than records
it stores, so their tables are built from names the API spells in `snake_case`.
Each one prints those names as words — `most_matches_played` becomes
`Most Matches Played` — and every one of them keeps the exact document the API
sent under `--format json`.
...
list cannot be had still gets its labels from the match keys. Predicted Winner
is `red` or `blue`, lowercase, the way every other winner column writes an
alliance. Red Score and Blue Score are the model's own numbers, always to two
decimals, and so is Confidence, the model's probability for the winner it
picked — a column of numbers is read by comparing them, and a ragged decimal
point makes that a digit count. A match the model has
...
rather than a prediction of red. `--rankings` switches to the predicted
...
`tba event export <key>` writes everything the API knows about an event to
files, one per dataset.

**`--to` is required.** It is the only thing that decides the format, and there
are three values:
...
tba event matches 2024cthar --jq '.[].key'
tba district list --year 2024 --format csv
tba event rankings 2024cthar --format markdown
```

For single-object commands (e.g. `team view`), tabular formats like
`csv`/`tsv`/`markdown` fall back to `json`. Use `--jq` to filter with jq
expressions; `-r` (`--raw-output`) drops the quotes around string results, like
`jq -r`.

Columns are measured in terminal cells, so a CJK or emoji nickname still lines
up in `table` output.
...
tba event rankings 2024cthar --columns 1,2 --no-headers
...
An empty answer is an answer. A listing with no rows — a schedule that has not
been posted, a filter that matched nothing, a search that found nobody, an event
the prediction model skipped — prints nothing on stdout, says why on stderr, and
exits 0. Under `--format json` it prints `[]` (or `null` for a single-object
command such as `team next`) and says nothing at all, so a pipeline keeps
parsing. Only a genuine failure — a bad key, a missing resource, a dead
network — is a non-zero exit.
...
`--no-cache` asks again. `tba status` prints that status document, so
`tba status --jq .current_season` is the season everything else defaults to.

`tba team awards` is the exception: its `--year` defaults to every year a team
has won anything. `tba open --year` is a different thing again — it picks a
team's season page instead of the team's overview page, and has no default.
...
Auth keys are stored separately per base URL, so prod and local credentials
don't conflict.

## Caching and network

Responses are cached locally and revalidated with `If-None-Match` /
`If-Modified-Since` on each request. When the server returns `304 Not Modified`,
the cached body is used. Cache files live in a `v1/` subdirectory of
`$XDG_CACHE_HOME/tba` (defaults to `~/.cache/tba`), so `tba cache clear` can
only remove files `tba` wrote.
...
```
tba event matches 2024cthar --timeout 30s
...
**`--` ends the flags.** An argument that starts with a dash is otherwise read as a flag, which matters for `team search`, whose query is free text:

```
$ tba team search -blue
...
`tba completion <shell>` prints a completion script for `bash`, `zsh`, `fish`,
or `powershell`. Common install paths:
...
What gets completed, and why it never goes to the network, is described under
[Tab completion](#tab-completion).

Man pages and markdown documentation are generated from the command tree
itself, so they cannot drift from the flags the binary has. Release archives
carry the man pages already built; to produce them locally:
...
**Release notes.** There is no `CHANGELOG.md`. GoReleaser writes each release's
notes from the commit messages since the previous tag, grouped by their
conventional-commit prefix into **Features** (`feat:`), **Fixes** (`fix:`),
**Documentation** (`docs:`) and **Other**, with `chore(deps)`, `ci`, `test` and
merge commits filtered out. A commit message is therefore the release note, so
it is worth writing as one.

**Cutting a release** (maintainers). Push a tag:
