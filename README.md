...
Piping a watch into something that stops reading — `tba event watch 2024cthar | head -1` — stops it too: a quiet poll writes nothing, so a broken pipe would never surface on its own, and the watch checks stdout before each poll and each wait instead, exiting 141 without a word. (Unix only; on Windows it keeps polling until `--for` elapses.)
